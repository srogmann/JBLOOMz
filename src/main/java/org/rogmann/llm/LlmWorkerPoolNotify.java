 ...
 */
public class LlmWorkerPoolNotify implements LlmExecutor {
	/** logger */
	static final Logger LOGGER = Logger.getLogger(LlmWorkerPoolNotify.class.getName());
	...
	public LlmWorkerPoolNotify(final int nThreads) {
	...
	class LlmWorkerThread extends Thread {
		...
		public void run() {
			...
			if (LOGGER.isLoggable(Level.FINER)) {
				LlmWorkerPoolNotify.LOGGER.finer("Thread " + idxThread + ": terminates (count " + cnt + ")");
			}
		}
	}
